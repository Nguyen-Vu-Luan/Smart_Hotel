...
const Checkout = ({ route, navigation }) => {
    ...
    // Thêm state này vào Checkout.js
                        ...
                        <WebView 
                            ...
                            onNavigationStateChange={(navState) => {
                                ...
                                if (navState.url.includes('vnpay-callback')) {
                                    ...
                                    if (isProcessing) return; // Nếu đã đang xử lý thì không làm gì cả
                                    ...
                                    if (navState.url.includes('vnpay_ResponseCode=00')) {
                                        ...
                                        // Xử lý hủy: Kiểm tra nếu thực sự là mã hủy mới gọi API
                                        if (navState.url.includes('vnp_ResponseCode=')) {
                                            cancelBooking().then(() => {
                                                setShowWebView(false);
                                                Alert.alert("Thông báo", "Giao dịch đã bị hủy. Đơn đặt phòng của bạn đã được chuyển sang trạng thái đã hủy.", [
