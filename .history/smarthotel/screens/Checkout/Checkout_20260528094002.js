...
const Checkout = ({ route, navigation }) => {
    ...
    // 2. Logic xử lý thanh toán thực tế
    const handlePaymentProcess = async () => {
        setLoading(true);
        try {
            // Bước 1: Tạo Booking với thông tin thực
            const bookingResponse = await API.post('/bookings/', {
                check_in_date: checkIn,
                check_out_date: checkOut,
                special_requests: "Khách hàng yêu cầu check-in sớm",
                details: [{ room: room.id }] // Dùng ID phòng thực
            });

            const createdBookingId = bookingResponse.data.id;

            // Bước 2: Thanh toán COD hoặc VNPAY
            if (paymentMethod === 'cod') {
