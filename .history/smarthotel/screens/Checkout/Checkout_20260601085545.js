            ...
            <Modal visible={showWebView} animationType="slide">
                <View style={{ flex: 1, marginTop: 50 }}>
                    ...
                    {vnpayUrl && (
                        ...
                        //         // Chỉ xử lý nếu chưa xử lý xong và đúng URL callback
                        //         if (!isHandled && url.includes('vnpay-callback')) {
                        //             setIsHandled(true); // Khóa ngay để tránh chạy lại

                        //             if (url.includes('vnp_ResponseCode=00')) {
                        //                 // BƯỚC 1: Gọi API cập nhật trạng thái lên Server trước
                        //                 try {
                        //                     const token = await AsyncStorage.getItem('token');
                        ...
                        //                     // BƯỚC 2: Sau khi Server đã đổi thành CONFIRMED, mới chuyển trang
                        //                     setShowWebView(false);
                        //                     navigation.replace('PaymentResult', {
                        //                         bookingId, totalPrice, status: 'Thanh toán thành công',
                        //                         roomName: room.name, roomTypeName: room.typeName,
                        //                         basePrice: room.price, checkIn, checkOut,
                        //                         paymentMethod: 'vnpay', services
                        //                     });
                        //                 } catch (error) {
                        //                     console.log("CHI TIẾT LỖI API:", error.response ? error.response.data : error.message);
                        //                     Alert.alert("Lỗi", "Đã thanh toán nhưng không thể đồng bộ đơn hàng.");
                        //                 }
                        //             } else {
                        //                 // Xử lý khi hủy/lỗi
                        //                 setShowWebView(false);
                        //                 Alert.alert("Thông báo", "Giao dịch không thành công.");
                        //                 navigation.goBack();
                        //             }
                        //         }
                        //     }}
                        ...
                        <WebView 
                            ...
                            onShouldStartLoadWithRequest={(request) => {
                                ...
                                // Bắt sự kiện khi VNPAY điều hướng về trang callback
                                if (url.includes('vnpay-callback')) {
                                    if (!isHandled) {
                                        setIsHandled(true);
                                        setShowWebView(false); // Ẩn WebView ngay lập tức để không hiện trang API

                                        if (url.includes('vnp_ResponseCode=00')) {
                                            // Gọi API xác nhận trạng thái booking
                                            API.post(`/bookings/${bookingId}/confirm-vnpay/`)
                                                .then(() => {
                                                     navigation.replace('PaymentResult', {
                        //                     bookingId: bookingId,
                        //                     totalPrice: totalPrice,
                        ...
                        //                 });
                                                ...
                                                .catch(() => {
                                                    // Dù API lỗi, vẫn cho qua vì DB đã được update qua IPN rồi
                                                    navigation.replace('PaymentResult', { 
                                                        bookingId, status: 'Thanh toán thành công', ...params 
                                                    });
                                                });
                                        } else {
                                            Alert.alert("Thông báo", "Giao dịch không thành công.");
                                    ...
                                    return false; // Chặn không cho WebView load tiếp trang callback
                                }
                                return true; // Cho phép load các trang thanh toán bình thường
                            }}
                        />
