...
const Checkout = ({ route, navigation }) => {
    // Nhận dữ liệu truyền từ Booking.js sang
    const { room, checkIn, checkOut, totalPrice, services } = route.params;
    const user = useContext(MyUserContext); // 3. Lấy user từ context
    ...
    const handlePaymentProcess = async () => {
        if (!user || !user.token) {
            Alert.alert("Lỗi", "Vui lòng đăng nhập để thực hiện đặt phòng!");
            navigation.navigate("Login");
            return;
        }
        setLoading(true);
        try {
            // Kiểm tra xem đã có token trong bộ nhớ chưa
            const token = await AsyncStorage.getItem('access_token');
            if (!token) {
                Alert.alert("Thông báo", "Bạn cần đăng nhập để đặt phòng!");
                navigation.navigate("Login");
                return;
            }

            // BƯỚC 1: Gọi API tạo đơn đặt phòng
            const bookingResponse = await API.post('/bookings/', {
            ...
            // Gọi API VNPAY (cũng cần Token nếu Backend bắt)
            const paymentResponse = await API.post('/payments/create-vnpay/', { 
                booking_id: createdBookingId 
            }, {
